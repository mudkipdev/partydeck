...
pub fn resolve_handler_from_cli(mode: &LaunchMode) -> Result<Handler, String> {
    match mode {
        LaunchMode::Gui => Err("GUI mode does not specify a handler".to_string()),
        LaunchMode::Handler(uid) => crate::handler::find_handler_by_uid(uid)
            .ok_or_else(|| format!("Handler with UID '{}' not found", uid)),
        LaunchMode::Executable(exec, args) => Ok(Handler::new_from_cli(exec, args)),
    }
}

fn find_device_index(devices: &[InputDevice], identifier: &str) -> Option<usize> {
...
pub fn list_all_handlers() {
    let handlers = crate::handler::scan_handlers();
